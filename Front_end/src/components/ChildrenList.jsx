...
import { 
  ...
  FaTimes, FaChartLine, FaWeight, FaSyringe
} from "react-icons/fa";
...
const MedicalInfoModal = ({ isOpen, onClose, childName, childId }) => {
  ...
  const handleItemClick = (label) => {
    if (label === "Add Measurement") {
      navigate(`/Home/children/${childId}/add_measurement`);
    } else {
      onClose();
    }
  };
          ...
          <table className="nurture-table">
            ...
            <tbody>
              {filteredChildren.length > 0 ? (
                    ...
                    <tr key={child.id}>
                      <td className="child-info-cell">
                        <div className="child-avatar" style={{ borderColor: isBoy ? "#87CEEB" : "#FFC0CB" }}>
                          {child.profileImage ? (
                            <img src={`http://localhost:5000/uploads/${child.profileImage}`} alt="profile" style={{ width: '100%', height: '100%', borderRadius: '50%', objectFit: 'cover' }} />
                          ) : (
                        ...
                        <div className="child-details-stack">
                          ...
                          <div className="child-action-row">
                            ...
                            <button className="btn-mini-add" onClick={() => openModal(child.id, child.childName)}>
                              <FaPlus /> Add
                            </button>
                          </div>
                        </div>
                ...
                <tr>
                  <td colSpan="7" style={{ textAlign: "center", padding: "20px" }}>No children found.</td>
                </tr>
              )}
            </tbody>
          </table>
